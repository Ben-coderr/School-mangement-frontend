...
type Props = {
  type: 'create' | 'update';
  data?: Partial<ExamInputs & { id: number; lesson?: { id: number } }>;
...
const ExamForm: React.FC<Props> = ({
  ...
  const [lessons, setLessons] =
    useState<{ id: number; topic: string }[]>([]);
  ...
  /* ---------- submit ---------- */
  const submit = (values: ExamInputs) => {
    const payload: any = {
      title:    values.title,
  ...
  return (
    <form className="flex flex-col gap-8" onSubmit={handleSubmit(submit)}>
      ...
      <div className="flex flex-wrap gap-4 justify-between">
        {/* ID (read-only) shown only in update mode */}
        ...
        {isTeacher ? (
          <div className="flex flex-col gap-2 w-full md:w-[30%]">
            ...
            {lessonErr && (
              <p className="text-xs text-red-400">{lessonErr}</p>
            )}
            {errors.lessonId && (
